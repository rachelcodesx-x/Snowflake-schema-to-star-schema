...
FROM (
    SELECT 
        ...
        metadata$file_content_key as _stg_file_md5,
        current_timestamp as _copy_data_ts
    FROM @stage_sch.csv_stg/delta/location/ay-03-duplicate-data.csv t
)
--FILES = ('delta-day01-2rows-add.csv', 'delta-day02-2rows-update.csv','day-03-duplicate-data.csv')
FILE_FORMAT = (FORMAT_NAME = 'stage_sch.csv_file_format')
ON_ERROR = ABORT_STATEMENT;