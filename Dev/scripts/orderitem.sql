...
create or replace table stagesch.orderitem (
...
create or replace stream stagesch.orderitem_stm 
on table stagesch.orderitem
append_only = true
...
copy into stagesch.orderitem (orderitemid, orderid, menuid, quantity, price, 
...
from (
    ...
    from @sandbox.stage_sch.csv_stg/initial/order_item/order-Item-initial.csv t
)
...
CREATE OR REPLACE TABLE cleansch.order_item (
...
create or replace stream CLEANSCH.order_item_stm 
on table CLEANSCH.order_item
append_only = True
...
CREATE OR REPLACE TASK Common.task_orderitem_stage2clean
WAREHOUSE = adhoc_wh
SCHEDULE = 'USING CRON * * * * * Asia/Kolkata'
AS
MERGE INTO cleansch.order_item AS target
USING stagesch.orderitem_stm AS source
ON  
...
ALTER TASK Common.task_orderitem_stage2clean RESUME;
-- part-2
...
from (
    select 
        ...
        metadata$file_content_key as _stg_file_md5,
        current_timestamp as _copy_data_ts
    from @stage_sch.csv_stg/delta/order_item/day-02-order-item.csv t
)
