...
create or replace table stagesch.customeraddress (
...
create or replace stream stagesch.customeraddress_stm 
on table stagesch.customeraddress
append_only = true
comment = 'This is the append-only stream object on customer address table that only gets delta data';


select * from stagesch.customeraddress_stm;
copy into stagesch.customeraddress (addressid, customerid, flatno, houseno, floor, building, 
...
from (
    ...
    from @sandbox.stage_sch.csv_stg/initial/customer_address_book t
)
...
-- 2nd layer
CREATE OR REPLACE TABLE CLEANSCH.CUSTOMER_ADDRESS (
...
create or replace stream CLEANSCH.CUSTOMER_ADDRESS_STM
on table CLEANSCH.CUSTOMER_ADDRESS
append_only= True
comment = 'This is the stream object on customer address entity to track insert, update, and delete changes';

CREATE OR REPLACE TASK Common.task_custadd_stage2clean
WAREHOUSE = adhoc_wh
...
CREATE OR REPLACE TABLE CONSUMPTIONSCH.CUSTOMER_ADDRESS_DIM (
...
CREATE OR REPLACE TASK Common.task_custadd_clean2stage
WAREHOUSE = adhoc_wh
SCHEDULE = 'USING CRON * * * * * Asia/Kolkata'
AS
MERGE INTO 
...
ALTER TASK Common.Merge_Stage_To_Clean RESUME;
ALTER TASK Common.Merge_Clean_To_Consumption RESUME;
list @stage_sch.csv_stg/delta/customer_address_csv;
...
from (
    select 
        ...
        metadata$file_content_key as _stg_file_md5,
        current_timestamp as _copy_data_ts
    from @stage_sch.csv_stg/delta/customer_address_csv/day-02-customer-address-book.csv t
)
