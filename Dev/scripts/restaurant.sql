...
create or replace table stagesch.restaurant (
    restaurantid text,      
...
create or replace stream stagesch.restaurant_stm 
on table stagesch.restaurant
append_only = true
...
copy into stagesch.restaurant (restaurantid, name, cuisinetype, pricing_for_2, restaurant_phone, 
...
from (
    select 
     ...
     from @sandbox.stage_sch.csv_stg/initial/restuarant/restaurant-delhi+NCR.csv t
)
...
create or replace table cleansch.restaurant (
...
create or replace stream cleansch.restaurant_stm 
on table cleansch.restaurant
append_only= True
...
CREATE OR REPLACE TASK Common.task_restaurant_stagetoclean
SCHEDULE = 'USING CRON * * * * * Asia/Kolkata'
AS
MERGE INTO cleansch.restaurant AS target
USING (
    SELECT 
...
CREATE OR REPLACE TABLE CONSUMPTIONSCH.RESTAURANT_DIM (
...
select count(*) from CLEANSCH.RESTAURANT_STM;

-- merge statement
CREATE OR REPLACE TASK Common.task_restaurant_cleantoconsumption
SCHEDULE = 'USING CRON * * * * * Asia/Kolkata'
AS
MERGE INTO 
    CONSUMPTIONSCH.RESTAURANT_DIM AS target
USING 
    CLEANSCH.RESTAURANT_STM AS source
ON 
...
from (
    select 
        ...
        metadata$file_content_key as _stg_file_md5,
        current_timestamp() as _copy_data_ts
     from @sandbox.stage_sch.csv_stg/delta/restaurant/day-01-insert-restaurant-delhi+NCR.csv t
)
file_format = (format_name = 'sandbox.stage_sch.csv_file_format')
on_error = abort_statement;
...
ALTER TASK Common.task_restaurant_stagetoclean resume;
ALTER TASK Common.task_restaurant_cleantoconsumption resume;
