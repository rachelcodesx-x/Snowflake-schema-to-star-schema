...
CREATE OR REPLACE TABLE CONSUMPTIONSCH.DATE_DIM (
...
CREATE OR REPLACE TASK Common.datedim
WAREHOUSE = adhoc_wh
SCHEDULE = 'USING CRON * * * * * Asia/Kolkata'
as
insert into CONSUMPTIONSCH.DATE_DIM  
...
(
    ...
    from 
        my_date_dim_cte
    where 
        today_r > (select date(min(order_date)) from cleansch.orders)
)
select 
...
ALTER TASK Common.datedim RESUME;