...
create or replace table stagesch.delivery (
...
create or replace stream stagesch.delivery_stm 
on table stagesch.delivery
append_only = true
comment = 'this is the append-only stream object on delivery table that only gets delta data';


copy into stagesch.delivery (deliveryid,orderid, deliveryagentid, deliverystatus, 
...
from (
    ...
    from @sandbox.stage_sch.csv_stg/initial/delivery/delivery-initial-load.csv t
)
file_format = (format_name = 'sandbox.stage_sch.csv_file_format')
on_error = abort_statement;


CREATE OR REPLACE TABLE cleansch.delivery (
...
create or replace stream CLEANSCH.delivery_stm 
on table CLEANSCH.delivery
append_only= True
comment = 'This is the stream object on delivery agent table table to track insert, update, and delete changes';


CREATE OR REPLACE TASK Common.task_delivery_stage2clean
WAREHOUSE = adhoc_wh
SCHEDULE = 'USING CRON * * * * * Asia/Kolkata'
As
MERGE INTO 
    cleansch.delivery AS target
USING 
    stagesch.delivery_stm AS source
ON 
...
ALTER TASK  Common.task_delivery_stage2clean RESUME;
