create or replace table stagesch.location (
    locationid text,
    city text,
    state text,
    zipcode text,
...
;
create or replace table cleansch.restaurant_location (
...
create or replace stream stagesch.location_stm 
on table stagesch.location
append_only = true
comment = 'this is the append-only stream object on location table that gets delta data based on changes';

select * from stagesch.location;

create or replace stream cleansch.restaurant_location_stm 
on table cleansch.restaurant_location
append_only = true
comment = 'this is a standard stream object on the location table to track insert, update, and delete changes';

create or replace table consumptionsch.restaurant_location_dim (
...
CREATE OR REPLACE TASK Common.task_merge_stage_to_clean
WAREHOUSE = adhoc_wh
SCHEDULE = 'USING CRON * * * * * Asia/Kolkata'
AS
MERGE INTO cleansch.restaurant_location AS target
USING (
    SELECT 
...
ALTER TASK Common.task_merge_stage_to_clean resume;
ALTER TASK Common.task_merge_clean_to_consumption resume;



CREATE OR REPLACE TASK Common.task_merge_clean_to_consumption
WAREHOUSE = adhoc_wh
SCHEDULE = 'USING CRON 0 * * * * Asia/Kolkata'
AS
MERGE INTO 
        CONSUMPTIONSCH.RESTAURANT_LOCATION_DIM AS target
    USING 
        CLEANSCH.RESTAURANT_LOCATION_STM AS source
    ON 
...
copy into stagesch.location (locationid, city, state, zipcode, activeflag, 
...
from (
    select 
        ...
        current_timestamp as _copy_data_ts
    from @sandbox.stage_sch.csv_stg/delta/location/delta-day01-2rows-add.csv t
)
...
select * from stagesch.location_stm;
