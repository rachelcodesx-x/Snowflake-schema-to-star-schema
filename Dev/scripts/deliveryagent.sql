...
create or replace table stagesch.deliveryagent (
...
create or replace stream stagesch.deliveryagent_stm 
on table stagesch.deliveryagent
append_only = true
comment = 'This is the append-only stream object on delivery agent table that only gets delta data';


copy into stagesch.deliveryagent (deliveryagentid, name, phone, vehicletype, locationid, 
...
from (
    ...
    from @sandbox.stage_sch.csv_stg/initial/delivery_agent/delivery-agent-initial.csv t
)
...
CREATE OR REPLACE TABLE cleansch.delivery_agent (
...
create or replace stream CLEANSCH.delivery_agent_stm 
on table CLEANSCH.delivery_agent
append_only= True
comment = 'This is the stream object on delivery agent table table to track insert, update, and delete changes';


CREATE OR REPLACE TASK Common.task_deliveryagent_stage2clean
WAREHOUSE = adhoc_wh
SCHEDULE = 'USING CRON * * * * * Asia/Kolkata'
As
MERGE INTO cleansch.delivery_agent AS target
USING stagesch.deliveryagent_stm AS source
...
CREATE OR REPLACE TABLE consumptionsch.delivery_agent_dim (
...
CREATE OR REPLACE TASK Common.task_deliveryagent_clean2consumption
WAREHOUSE = adhoc_wh
SCHEDULE = 'USING CRON * * * * * Asia/Kolkata'
as
MERGE INTO consumptionsch.delivery_agent_dim AS target
USING CLEANSCH.delivery_agent_stm AS source
ON 
...
ALTER TASK Common.task_deliveryagent_stage2clean RESUME;
ALTER TASK Common.task_deliveryagent_clean2consumption RESUME;


-- part-2
list @stage_sch.csv_stg/delta/delivery_agent;
...
from (
    select 
        ...
        metadata$file_content_key as _stg_file_md5,
        current_timestamp as _copy_data_ts
    from @stage_sch.csv_stg/delta/delivery_agent/day-02-delivery-agent.csv t
)
