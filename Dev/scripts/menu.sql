...
create or replace table stagesch.menu (
...
create or replace stream stagesch.menu_stm 
on table stagesch.menu
append_only = true
...
copy into stagesch.menu (menuid, restaurantid, itemname, description, price, category, 
...
from (
    ...
    from @sandbox.stage_sch.csv_stg/initial/menu/menu-initial-load.csv t
)
...
CREATE OR REPLACE TABLE cleansch.menu (
...
create or replace stream CLEANSCH.menu_stm 
on table CLEANSCH.menu
append_only= True
comment = 'This is the stream object on menu table table to track insert, update, and delete changes';

CREATE OR REPLACE TASK Common.task_menu_stage2clean
WAREHOUSE = adhoc_wh
SCHEDULE = 'USING CRON * * * * * Asia/Kolkata'
AS
MERGE INTO cleansch.menu AS target
USING (
    SELECT 
...
CREATE OR REPLACE TABLE consumptionsch.menu_dim (
...
CREATE OR REPLACE TASK Common.task_menu_clean2consump
WAREHOUSE = adhoc_wh
SCHEDULE = 'USING CRON * * * * * Asia/Kolkata'
AS
MERGE INTO 
    consumptionsch.MENU_DIM AS target
USING 
    CLEANSCH.MENU_STM AS source
ON 
...
ALTER TASK Common.task_menu_clean2consump RESUME;
ALTER TASK Common.task_menu_stage2clean RESUME;
