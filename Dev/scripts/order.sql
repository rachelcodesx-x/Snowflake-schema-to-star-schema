...
create or replace table stagesch.orders (
...
create or replace stream stagesch.orders_stm 
on table stagesch.orders
append_only = true
comment = 'This is the append-only stream object on orders entity that only gets delta data';

list  @stage_sch.csv_stg/initial/order_csv;


copy into stagesch.orders (orderid, customerid, restaurantid, orderdate, totalamount, 
...
from (
    ...
    from @sandbox.stage_sch.csv_stg/initial/order_csv/orders-initial.csv t
)
file_format = (format_name = 'sandbox.stage_sch.csv_file_format')
on_error = abort_statement;


CREATE OR REPLACE TABLE CLEANSCH.ORDERS (
...
create or replace stream CLEANSCH.ORDERS_stm 
on table CLEANSCH.ORDERS
append_only= True
comment = 'This is the stream object on ORDERS table table to track insert, update, and delete changes';


CREATE OR REPLACE TASK Common.task_orders_stage2clean
WAREHOUSE = adhoc_wh
SCHEDULE = 'USING CRON * * * * * Asia/Kolkata'
AS
MERGE INTO CLEANSCH.ORDERS AS target
USING STAGESCH.ORDERS_STM AS source
...
ALTER TASK Common.task_orders_stage2clean RESUME;
