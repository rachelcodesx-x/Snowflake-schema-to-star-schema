...
create or replace table stagesch.customer (
    customerid text,                    -- primary key as text
    name text,                          -- name as text
    mobile text ,                        -- mobile number as text
...
create or replace stream stagesch.customer_stm 
on table stagesch.customer
append_only = true
...
copy into  stagesch.customer (customerid, name, mobile, email, loginbyusing, gender, dob, anniversary, 
...
from (
    ...
    from @sandbox.stage_sch.csv_stg/initial/customer/customers-initial.csv t
)
...
CREATE OR REPLACE TABLE CLEANSCH.CUSTOMER (
    ...
    MOBILE STRING(15) ,                                           -- Mobile number, accommodating international format
...
-- Stream object to capture the changes. 
create or replace stream CLEANSCH.customer_stm 
on table CLEANSCH.customer
append_only= True
comment = 'This is the stream object on customer entity to track insert, update, and delete changes';

CREATE OR REPLACE TASK Common.task_customer_stagetoclean
WAREHOUSE = adhoc_wh
SCHEDULE = 'USING CRON * * * * * Asia/Kolkata'
As
MERGE INTO CLEANSCH.CUSTOMER AS target
USING (
    SELECT 
...
-- create dim table 
CREATE OR REPLACE TABLE CONSUMPTIONSCH.CUSTOMER_DIM (
...
CREATE OR REPLACE TASK Common.task_customer_cleantoconsump
WAREHOUSE = adhoc_wh
SCHEDULE = 'USING CRON * * * * * Asia/Kolkata'
AS
MERGE INTO 
    CONSUMPTIONSCH.CUSTOMER_DIM AS target
USING 
    CLEANSCH.CUSTOMER_STM AS source
ON 
...
from (
    select 
        ...
        metadata$file_content_key as _stg_file_md5,
        current_timestamp as _copy_data_ts
    from @sandbox.stage_sch.csv_stg/delta/customer/day-01-insert-customer.csv t
)
file_format = (format_name = 'sandbox.stage_sch.csv_file_format')
on_error = abort_statement;
...
ALTER TASK Common.task_customer_cleantoconsump resume;
ALTER TASK Common.task_customer_stagetoclean resume;