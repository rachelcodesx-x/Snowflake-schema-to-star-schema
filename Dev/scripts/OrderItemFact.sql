...
CREATE OR REPLACE TABLE consumptionsch.order_item_fact (
...
CREATE OR REPLACE TASK Common.orderitemfact
WAREHOUSE = adhoc_wh
SCHEDULE = 'USING CRON * * * * * Asia/Kolkata'
as
MERGE INTO consumptionsch.order_item_fact AS target
USING (
    SELECT 
    ...
    FROM 
        cleansch.order_item_stm oi
    JOIN 
        cleansch.orders_stm o ON oi.Order_ID_fk = o.Order_ID
    JOIN 
        cleansch.delivery_stm d ON o.Order_ID = d.Order_ID_fk
    JOIN 
        consumptionsch.CUSTOMER_DIM c on o.Customer_ID_fk = c.customer_id
    JOIN 
        consumptionsch.CUSTOMER_ADDRESS_DIM ca on c.Customer_ID = ca.CUSTOMER_ID_fk
    JOIN 
        consumptionsch.restaurant_dim r on o.Restaurant_ID_fk = r.restaurant_id
    JOIN 
        consumptionsch.menu_dim m ON oi.MENU_ID_fk = m.menu_id
    JOIN 
        consumptionsch.delivery_agent_dim da ON d.Delivery_Agent_ID_fk = da.delivery_agent_id
    JOIN 
        consumptionsch.restaurant_location_dim rl on r.LOCATION_ID_FK = rl.location_id
    JOIN 
        CONSUMPTIONSCH.DATE_DIM dd on dd.calendar_date = date(o.order_date)
) AS source_stm
ON 
